...
//            }
//        }
        final Map<String, GenericAction> genericActionsMap = (Map<String, GenericAction>) allArguments[3];
        if (Objects.nonNull(genericActionsMap) && genericActionsMap.size() > 0) {
            genericActionsMap.forEach((k, v) -> {
                if (v instanceof EnhancedInstance) {
                    ElasticSearchEnhanceInfo elasticSearchEnhanceInfo = new ElasticSearchEnhanceInfo();
                    elasticSearchEnhanceInfo.setClusterName(clusterName);
                    parseRequestInfo(v, elasticSearchEnhanceInfo);
                    elasticSearchEnhanceInfo.setTransportAddressHolder(nodeService);
                    ((EnhancedInstance) v).setSkyWalkingDynamicField(elasticSearchEnhanceInfo);
                }
            });
        }
    }
